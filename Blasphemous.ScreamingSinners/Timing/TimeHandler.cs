...
namespace Blasphemous.ScreamingSinners.Timing;
...
internal partial class TimeHandler
{
    ...
    private void AddTimeable(string id, ITimeable timeable)
    {
        ...
        {
            Main.ScreamingSinners.LogError($"A timer with id {id} already exists.");
            return;
        }
        Main.ScreamingSinners.Log($"Adding timer: {id}");
        _timers.Add(id, timeable);
    }

    public void RemoveTimer(string id)
    {
        Main.ScreamingSinners.Log($"Removing timer: {id}");
        _timers.Remove(id);
    }
    ...
    private void RemoveTimeables(Func<ITimeable, bool> predicate)
    {
        ...
        {
            Main.ScreamingSinners.Log($"Stopping timer: {t.Key}");
    ...
    public void Update()
    {
        if (!Main.ScreamingSinners.LoadStatus.GameSceneLoaded || Core.Logic.Penitent == null)
            return;
